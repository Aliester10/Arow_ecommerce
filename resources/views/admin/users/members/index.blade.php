...
@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('admin.users.members.create') }}"
    ...
    <div class="mb-4">
        <form method="GET" action="{{ route('admin.users.members.index') }}" class="flex gap-2 max-w-xl">
            <input type="text" name="q" value="{{ $search }}" placeholder="Cari nama, email, atau perusahaan..."
            ...
            <table class="w-full table-auto">
                <thead>
                    <tr class="bg-gray-2 text-left dark:bg-gray-700">
                        ...
                        <th class="min-w-[240px] py-4 px-4 font-medium text-black dark:text-white">
                            Alamat
                        </th>
                    ...
                    @forelse ($members as $member)
                        <tr>
                            <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-gray-700 xl:pl-11">
                                <p class="text-black dark:text-white">{{ $member->nama_user }}</p>
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $member->email_user }}</p>
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $member->nama_perusahaan ?? '-' }}</p>
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $member->nomor_telepon ?? '-' }}</p>
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $member->alamat ?? '-' }}</p>
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
