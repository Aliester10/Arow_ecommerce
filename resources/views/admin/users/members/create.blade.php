...
@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('admin.users.members.index') }}"
    ...
    <div class="rounded-sm border border-stroke bg-white p-6 shadow-default dark:border-gray-700 dark:bg-gray-800">
        <form method="POST" action="{{ route('admin.users.members.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                <label class="mb-2.5 block font-medium text-black dark:text-white">Nama Perusahaan</label>
            ...
            <div>
                ...
                <textarea name="alamat" rows="3"
                    class="w-full rounded-md border border-stroke bg-transparent py-3 px-5 text-black outline-none focus:border-primary dark:border-gray-700 dark:text-white">{{ old('alamat') }}</textarea>
            </div>
