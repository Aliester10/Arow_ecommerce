    ...
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            <li class="inline-flex items-center">
                <a href="{{ route('admin.sub-categories.index') }}" class="text-gray-700 hover:text-brand-500">
                    Sub Kategori
                </a>
            </li>
            <li>
                <div class="flex items-center">
                    ...
                    <span class="text-gray-500">Edit Sub Kategori</span>
                </div>
            </li>
        </ol>
    </nav>
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-stroke dark:border-gray-700">
    <div class="px-6 py-4 border-b border-stroke dark:border-gray-700">
        <h3 class="text-lg font-semibold text-black dark:text-white">Edit Sub Kategori: {{ $subCategory->nama_subkategori }}</h3>
    </div>
    
    <form method="POST" action="{{ route('admin.sub-categories.update', $subCategory) }}" class="p-6">
        @csrf
        @method('PUT')
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="nama_subkategori" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            ...
            <div>
                <label for="id_kategori" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                ...
                <select name="id_kategori" 
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id_kategori }}" {{ old('id_kategori', $subCategory->id_kategori) == $category->id_kategori ? 'selected' : '' }}>
