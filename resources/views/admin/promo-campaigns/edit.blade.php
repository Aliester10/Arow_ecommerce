...
@section('content')
    ...
    <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-gray-700 dark:bg-gray-800 sm:px-7.5 xl:pb-1">
        <form action="{{ route('admin.promo-campaigns.update', $promoCampaign) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $promoCampaign->title) }}" required placeholder="Contoh: Promo Ramadhan"
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $promoCampaign->slug) }}" required readonly placeholder="promo-ramadhan"
                ...
                <div>
                    ...
                    <input type="text" name="subtitle" value="{{ old('subtitle', $promoCampaign->subtitle) }}" placeholder="Contoh: Hemat s/d 50% untuk furniture"
                        class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:border-primary">
                </div>
                ...
                <div>
                    <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                        Banner Promo (Pilih file baru untuk mengganti)
                    </label>
                    ...
                    @if($promoCampaign->banner)
                        <div class="mt-3">
                            ...
                            <img src="{{ asset('storage/images/' . $promoCampaign->banner) }}" alt="Current Banner" class="h-20 w-auto rounded-lg object-cover border border-stroke">
                        </div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" id="start_date" name="start_date" value="{{ old('start_date', $promoCampaign->start_date ? $promoCampaign->start_date->format('Y-m-d\TH:i') : '') }}" required
                        class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:border-primary">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" id="end_date" name="end_date" value="{{ old('end_date', $promoCampaign->end_date ? $promoCampaign->end_date->format('Y-m-d\TH:i') : '') }}" required
                        class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:border-primary">
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="description" rows="3" placeholder="Masukkan deskripsi promo..."
                    class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:border-primary">{{ old('description', $promoCampaign->description) }}</textarea>
            </div>
            ...
            <div class="mt-6">
                ...
                <div class="flex gap-4">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" name="status" value="aktif" class="mr-2" {{ old('status', $promoCampaign->status) === 'aktif' ? 'checked' : '' }}>
                        Aktif
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" name="status" value="nonaktif" class="mr-2" {{ old('status', $promoCampaign->status) === 'nonaktif' ? 'checked' : '' }}>
                        Nonaktif
                    </label>
                </div>
            </div>
                            ...
                            @foreach($products as $product)
                                @php
                                    ...
                                    if (is_array($oldSelected)) {
                                        ...
                                        $oldType = old("discount_type.{$product->id_produk}", 'percent');
                                        $oldVal = old("discount_value.{$product->id_produk}", 0);
                                    } else {
                                        $isOldChecked = in_array($product->id_produk, $selectedProductIds);
                                        $oldType = isset($selectedProductsPivot[$product->id_produk]) ? $selectedProductsPivot[$product->id_produk]['discount_type'] : 'percent';
                                        $oldVal = isset($selectedProductsPivot[$product->id_produk]) ? $selectedProductsPivot[$product->id_produk]['discount_value'] : 0;
                                    }
                                @endphp
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initial previews for pre-checked database/old entries
        ...
        function checkOverlappingProducts() {
            ...
            document.querySelectorAll('.product-card').forEach(function(card) {
                ...
                // Only reset checkboxes that are NOT disabled by other means or keep original behavior
