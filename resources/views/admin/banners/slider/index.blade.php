...
@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('admin.slider-banners.create') }}"
            ...
            <i class="fas fa-plus mr-2"></i> Tambah Slider Banner
        </a>
    </div>
        ...
        <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div class="flex items-center">
                ...
                <div class="text-sm text-blue-700">
                    <strong>Slider Banner:</strong> Banner ini hanya ditampilkan di halaman utama sebagai slider dan tidak dapat diklik. 
                    Fokus utama adalah visual dan tampilan berurutan.
                </div>
            </div>
        </div>
                    ...
                    @forelse($banners as $banner)
                        <tr>
                                ...
                                <div class="h-20 w-40 rounded-md">
                                    @if(file_exists(public_path('storage/images/' . $banner->gambar_slider_banner)))
                                        <img class="h-full w-full object-cover rounded-md border"
                                            src="{{ asset('storage/images/' . $banner->gambar_slider_banner) }}" alt="Slider Banner" />
                                    @else
                            ...
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $banner->gambar_slider_banner }}</p>
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white font-medium">{{ $banner->title ?? '-' }}</p>
                                <p class="text-sm text-gray-500">Slider Banner</p>
                                @if($banner->subtitle)
                                    <p class="text-xs text-gray-400">{{ $banner->subtitle }}</p>
                                @endif
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $banner->position }}</p>
                            </td>
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                @if($banner->active)
                            ...
                            <td class="border-b border-[#eee] py-5 px-4 dark:border-gray-700">
                                <div class="flex items-center space-x-3.5">
                                    <a href="{{ route('admin.slider-banners.edit', $banner->id_slider_banner) }}"
                                        class="hover:text-primary" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.slider-banners.destroy', $banner->id_slider_banner) }}" method="POST"
                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus slider banner ini?');"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="border-b border-[#eee] py-5 px-4 dark:border-gray-700 text-center">
                                Tidak ada slider banner ditemukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
