...
@section('content')
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-gray-700 dark:bg-gray-800">
            ...
            <form action="{{ route('admin.payment_accounts.update', $paymentAccount->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="p-6.5">
                    <div class="mb-4.5">
                        <label class="mb-2.5 block text-black dark:text-white">Nama Bank <span class="text-meta-1">*</span></label>
                    ...
                    <div class="mb-4.5">
                        <label class="mb-2.5 block text-black dark:text-white">Nomor Rekening <span class="text-meta-1">*</span></label>
                        <input type="text" name="account_number" value="{{ old('account_number', $paymentAccount->account_number) }}" required
                            class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                    </div>

                    <div class="mb-4.5">
                        <label class="mb-2.5 block text-black dark:text-white">Nama Pemilik Rekening <span class="text-meta-1">*</span></label>
                        <input type="text" name="account_holder" value="{{ old('account_holder', $paymentAccount->account_holder) }}" required
                            class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative z-20 bg-transparent dark:bg-form-input">
                            <select name="is_active" required
                                ...
                                <option value="1" {{ (string) old('is_active', (int) $paymentAccount->is_active) === '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ (string) old('is_active', (int) $paymentAccount->is_active) === '0' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
