...
<div class="container mx-auto px-4">
                                ...
                                <tr>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <div class="h-16 w-16 flex-shrink-0 bg-gray-100 rounded-md overflow-hidden border border-gray-200 flex items-center justify-center mr-4">
                                                <i class="fas fa-box text-gray-400 text-2xl"></i>
                                            </div>
                                            <div>
                                                <a href="{{ route('products.show', $detail->produk->id_produk) }}" class="font-medium text-gray-800 hover:text-orange-600 line-clamp-1">
                                                    {{ $detail->produk->nama_produk }}
                                                </a>
                                                <div class="text-xs text-gray-500 mt-1">{{ $detail->produk->brand->nama_brand ?? 'Brand' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <form action="{{ route('cart.update', $detail->id_cart_detail) }}" method="POST" class="inline-flex items-center border border-gray-300 rounded-md">
                                            @csrf
                                            @method('PUT')
                                            <input type="number" name="quantity" value="{{ $detail->qty_cart }}" min="1" class="w-12 text-center text-sm focus:outline-none border-none p-1" onchange="this.form.submit()">
                                        </form>
                                    </td>
    ...
    @else
        <div class="text-center py-16 bg-white rounded-lg shadow-sm border border-gray-100">
            ...
            <a href="{{ route('home') }}" class="inline-block px-6 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition">
                Mulai Belanja
            </a>
        </div>
    @endif
